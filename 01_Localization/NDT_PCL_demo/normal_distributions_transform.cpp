...
void visualizer(pcl::PointCloud<pcl::PointXYZ>::Ptr target_cloud, pcl::PointCloud<pcl::PointXYZ>::Ptr output_cloud)
{
    ...
    // Coloring and visualizing target cloud (red).                 点云1 cloud1
    pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> target_color(target_cloud, 255, 0, 0);
    viewer_final->addPointCloud<pcl::PointXYZ>(target_cloud, target_color, "target cloud");
    viewer_final->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 1, "target cloud");


    // color and visualizing transformed input cloud (green).       点云2 cloud2
...
{
    // 步骤1：读取点云信息 读取PCD文件中的点云信息
    pcl::PointCloud<pcl::PointXYZ>::Ptr target_cloud = read_cloud_point(argv[1]);           // 目标点云,点云1
