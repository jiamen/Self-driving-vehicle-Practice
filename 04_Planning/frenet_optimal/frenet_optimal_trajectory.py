...
TARGET_SPEED = 30.0 / 3.6   # 目标速度（即纵向的保持速度） [m/s]
D_T_S = 5.0 / 3.6           # 目标速度采样间隔 [m/s]
N_S_SAMPLE = 1              # Sampling number of target speed  目标速度的采样数
ROBOT_RADIUS = 2.0          # robot radius [m]   机器人半径
...
class quintic_polynomial:
    def __init__(self, xs, vxs, axs, xe, vxe, axe, T):
        ...
        """ 下面的操作是为了根据给定初始值xs，vxs等求出本类（五次多项式）的系数 """
        # 根据课本第189页矩阵方程得到α_d0, α_d1， α_d2
        self.a0  = xs
        self.a1  = vxs
        self.a2  = axs / 2.0
    ...
    # 计算控制点 P189页 d(t_0)计算公式    Jerk问题的解是p(t)，可以使用5次多项式来表示
    ...
    # 计算一次导数   derivate: 导数  P189页 d(t_0)'计算公式
    ...
    # 计算二次导数            P189页 d(t_0)'' 计算公式
    def calc_second_derivative(self, t):
        xt = 2 * self.a2 + 6 * self.a3 * t + 12 * self.a4 * t**2 + 20 * self.a5 * t**3

        return xt

    # 计算三次导数：P189页自己写的三次求导公式
    ...
    for di in np.arange(-MAX_ROAD_WIDTH, MAX_ROAD_WIDTH, D_ROAD_W):
        ...
        for Ti in np.arange(MINT, MAXT, DT):
            ...
            for tv in np.arange(TARGET_SPEED-D_T_S*N_S_SAMPLE, TARGET_SPEED+D_T_S*N_S_SAMPLE, D_T_S):
                ...
                lon_qp = quartic_polynomial(s0, c_speed, 0.0, tv, 0.0, Ti)  # 四次多项式
...
def frenet_optimal_planning(csp, s0, c_speed, c_d, c_d_d, c_d_dd, ob):
    ...
    fplist = check_paths(fplist, ob)                # 对路径进行检查
