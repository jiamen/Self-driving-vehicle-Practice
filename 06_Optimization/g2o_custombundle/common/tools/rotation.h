...
template<typename T>
inline T DotProduct(const T x[3], const T y[3])     //返回每个向量坐标点乘的结果
{
...
// Converts from a angle anxis to quaternion : 旋转向量到四元数的转换
template<typename T>
...
template < typename T >
inline void QuaternionToAngleAxis(const T* quaternion, T* angle_axis)    //四元数转换成角轴
{
...
template<typename T>
inline void AngleAxisRotatePoint(const T angle_axis[3], const T pt[3], T result[3])     // 世界坐标转换成相机坐标
{
    const T theta2 = DotProduct(angle_axis, angle_axis);        // 求每个数的平方和
